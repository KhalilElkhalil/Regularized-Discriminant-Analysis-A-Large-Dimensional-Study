...
% p_vect = 400:20:500;
lambda_vect = 0:0.1:1;
gamma_vect = 0.1:0.1:0.9;

Num = 100;
true_error_rda = zeros(length(lambda_vect),length(gamma_vect)) ; % RDA true error
theory_error_rda = zeros(length(lambda_vect),length(gamma_vect)) ; 
Theory_error_rda = zeros(length(lambda_vect),length(gamma_vect)) ;
...
%         arg0 = (xi_0 - traceB0)/sqrt(2*trace_B0B0+4/p*theory_y0'*theory_y0);
%         arg1 = (xi_1 - traceB1)/sqrt(2*trace_B1B1+4/p*theory_y1'*theory_y1);   
%         arg0 = (xi_0 - traceB0)/sqrt(2/p*trace(B0*B0)+4*y0y0);
%         arg1 = (xi_1 - traceB1)/sqrt(2/p*trace(B1*B1)+4*y1y1);
...
theory_error_rda(i,jj) = err_rda_Theory/Num;
true_error_rda(i,jj) = error_rda/n_test/Num;
%Theory_error_rda(i) = Err_rda_theory/Num;
        end
    end
toc
figure;
surf(lambda_vect,gamma_vect,theory_error_rda');
colormap hsv
colorbar
xlabel('lambda')
ylabel('gamma') ;
zlabel('classfication error');
title('p');
%legend({'asymptotic theory  RDA','true RDA'},'Fontsize',12);
grid on;
